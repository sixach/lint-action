...
import {normalizeDates, normalizePaths, createTmpDir} from '../test-utils'
import eslintParams from './params/eslint'
import eslintTypescriptParams from './params/eslint-typescript'
import stylelintParams from './params/stylelint'
...
describe.each(linterParams)(
  ...
  (
    ...
    beforeAll(async () => {
      await expect(
        linter.verifySetup(projectTmpDir, commandPrefix)
    ...
    describe.each([
      ...
      const expected = autoFix
        ? getFixParams(projectTmpDir)
        : getLintParams(projectTmpDir)
      ...
      if (!(autoFix && linter.name === 'WP-Scripts Lint JS')) {
        ...
        test(`${linter.name} parses ${lintMode} output correctly`, () => {
          const lintResult = linter.parseOutput(
            projectTmpDir,
            expected.cmdOutput
          )
