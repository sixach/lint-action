  ...
  try {
    ...
    for (const [linterId, linter] of Object.entries(linters)) {
      ...
      if (core.getInput(linterId) === 'true') {
        ...
        core.info(
          `Will use ${linter.name} to check the files with extensions ${fileExtList}`
        )

        // Lint and optionally auto-fix the matching files, parse code style violations
        core.info(
          `Linting ${
            autoFix ? 'and auto-fixing ' : ''
        ...
        if (autoFix) {
          ...
          if (git.hasChanges()) {
            git.commitChanges(
              commitMessage.replace(/\${linter}/g, linter.name),
              skipVerification
            )
