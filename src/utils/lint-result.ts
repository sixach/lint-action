 ...
 */
function initLintResult(): LintResult {
  return {
 ...
 */
function getSummary(lintResult: LintResult): string {
  ...
  if (nrWarnings > 0 && nrErrors > 0) {
    return `${nrErrors} error${
      nrErrors > 1 ? 's' : ''
    } and ${nrWarnings} warning${nrWarnings > 1 ? 's' : ''}`
  }
  if (nrErrors > 0) {
...
export {getSummary, initLintResult}
